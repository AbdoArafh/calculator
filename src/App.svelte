<script lang="ts">
  ...
  const performOperation = (operation?: ArthemticOperation) => {
    ...
    if (prev_num) {
      num = window
        .eval(
          `${stringToNumber(prev_num)} ${latest_operation} ${stringToNumber(
            num
          )}`
        )
        .toString();
...
<main class="main">
  ...
  <div class="body">
    {#each buttons as button}
      <button
        class={classNames("button", { elongated: button.elongated })}
        on:click={button.callback}>{button.symbol}</button
      >
    {/each}
  </div>
</main>
